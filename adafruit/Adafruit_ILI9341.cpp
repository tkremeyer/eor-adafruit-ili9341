...
static inline void spi_begin(void) {
  ...
  // max speed!
  SPIc::beginTransaction(SPISettings(16000000, MSBFIRST, SPI_MODE0));
#else
    // max speed!
  SPIc::beginTransaction(SPISettings(24000000, MSBFIRST, SPI_MODE0));
#endif
}
...
#ifndef SPI_HAS_TRANSACTION
    SPIc::setBitOrder(MSBFIRST);
    SPIc::setDataMode(SPI_MODE0);
  #if defined (_AVR__)
    SPIc::setClockDivider(SPI_CLOCK_DIV2); // 8 MHz (full! speed!)
    mySPCR = SPCR;
  #elif defined(TEENSYDUINO)
    SPIc::setClockDivider(SPI_CLOCK_DIV2); // 8 MHz (full! speed!)
  #elif defined (__arm__)
    SPIc::setClockDivider(11); // 8-ish MHz (full! speed!)
  #endif
#endif
  } else {
